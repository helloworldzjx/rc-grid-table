...
这份文档记录 `ConfigProvider`、`Theme`、Table 样式 token、内部 antd `Spin` / `Empty` 桥接的设计约束。
...
- 内部 `Spin` 和 `Empty` 的 `prefixCls` 必须跟随最终 Table 前缀。
...
项目内部使用了 antd `Spin` 和 `Empty`，因此需要把本项目主题桥接到 antd。
...
Table 内部继续使用 antd `Spin` 和 `Empty`，但前缀必须走 Table 体系：
...
只要当前 `ConfigProvider` 显式设置了 `theme`、`themeMode` 或 `cssVar`，就应该桥接 antd `ConfigProvider`，让内部 antd `Spin` / `Empty` 跟随本项目主题。
...
- 内部 antd `Spin` / `Empty` 继续继承外部 antd 主题。
...
- `ConfigProvider prefixCls` 可以改变 Table、Spin、Empty 前缀。
