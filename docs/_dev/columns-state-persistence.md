...
- 数据面：`columnsState` 保存当前可渲染列的完整外观状态，包括 `order`、`visible`、`fixed`、`width`、`widthManuallyChanged`、`autoWidthLocked` 等可恢复数据。
...
- 列是否存在，以及该列是否参与当前渲染。`hidden === true` 的列节点及其子树不会进入当前可渲染列结构，也不会被保存成 `visible: false`。
- 列的父子关系、分组结构、声明顺序和业务属性。
- `resizeDisabled`、`dragSortDisabled`、`resizeMinWidth` 等运行时声明。
- 内置列的结构属性，例如展开列、选择列、行拖拽列。

持久化 `ColumnState` 负责表达可恢复的外观状态：

- `order`：同级列顺序。它可能来自用户拖拽，也可能来自当前声明顺序归一化后的默认值。`order` 表达当前列状态树中同一父节点下的完整顺序，不只表达当前可见列顺序。
- `visible`：显隐状态。用户隐藏列通过 `visible: false` 表达；当前列声明 `hidden === true` 属于结构排除，该节点及其子树不进入 `ColumnState`。
- `fixed`：固定状态。这里指 `ColumnState` 和 ref API 的内部状态语义；`fixed: false` 表示用户或状态显式取消固定，不代表外部 `columns.fixed` 可以直接声明 `false`。
- `width`：用户拖拽、placeholder auto fill 或运行时宽度补齐后的可恢复列宽。
- `widthManuallyChanged`：宽度是否被用户手动拖拽调整过；placeholder auto fill 和 runtime width hydration 不应把它置为 `true`。
- `autoWidthLocked`：宽度是否已经由一次用户宽度提交消费过剩余空间。它用于阻止后续运行时布局再次自动二次分配宽度，可以随完整 `columnsState` 保存和恢复，但不属于 `info.patches` 的用户操作字段。
- `children`：仅用于保存同一棵列状态树中的层级结果。
- `dataIndex`：辅助识别信息，不作为结构权威来源。

`resizeMinWidth` 不属于持久化状态。它只来自当前 `columns`，类型只支持 `number`，用于运行时限制拖拽最小宽度，不参与持久化 `width` 的恢复校准。
...
`resizeMinWidth` 是例外。即使旧 storage 中仍然带有该字段，当前逻辑也不再恢复它，也不会用它抬高已保存的 `width`。恢复 `width` 时优先尊重 storage 中的可恢复宽度；`resizeMinWidth` 只在运行时拖拽阶段作为最小宽度约束。

无论当前列是否声明 `resizeMinWidth`，该下限只参与运行时拖拽计算，不会写入 `ColumnState`，也不会在恢复旧 `width` 时把宽度抬高。拖拽阶段的实际下限会被当前列宽封顶，避免历史宽度小于当前下限时发生跳变。
...
列状态管线里需要区分外部重置身份、外部初始化快照、内部状态源和运行时渲染结果。它们的职责不同，不能互相替代：

- `columnsStateKey` 是外部重置身份。它只回答“是否要重新消费一份初始化快照”，不表达当前 committed state 的版本，也不能用来判断某份渲染用列结构是否来自最新 committed value。拖拽列宽、列显隐、排序、内部归一化、runtime width hydration、UI 开关变化都不会改变 `columnsStateKey`。
...
- 最终快照由 committed state 或 preview draft 形成的 active state 生成，包括完整 `columnsState`、排序后的完整列状态、实际渲染树、flatten columns 和宽度结果。
- 运行时渲染结果来自最终快照中的实际渲染树和宽度结果。它不是状态源，不能反过来替代 committed state。
...
committed columnsState + preview draft 形成 active state
active state 生成最终快照和运行时渲染结果
...
运行时渲染结果只应由当前 active state 生成。active state 可以是 committed state，也可以是 preview draft 覆盖后的状态；它必须先经过当前 columns 归一化、排序、宽度分配和 runtime width hydration，形成一份最终快照。最终快照同时包含完整 `columnsState`、排序后的完整列状态，以及实际渲染用的 `treeColumns`、`flattenColumns` 和宽度结果。`onColumnsStateReady`、`onColumnsStateChange` 使用完整 `columnsState`；渲染层和 `viewState` 使用实际渲染结果，因此可以过滤 `visible: false` 的列。

runtime width hydration 可以借助本轮根据当前 columns 和 active state 计算出的候选宽度结果。如果 hydration、rebase 或初始化发现 active state 与最终完整快照不一致，应先更新对应内部状态并结束本轮，不能继续发布基于旧状态的渲染结果。初始化、用户提交或 preview save 已经在同一个工作流里得到最终快照时，可以直接用这份最终快照发布渲染结果和 ready/change 数据，不需要再通过下层 effect 二次分配宽度。这样可以避免“先用旧状态生成渲染结果，再用渲染结果反写状态”的循环。

## 默认值规则

合并结果必须补齐表格运行和持久化所需的基础状态：

- 可见性缺省为可见；用户显隐偏好通过 `visible: false` 表达。当前列声明 `hidden === true` 时，该列节点及其子树已经在当前可渲染列结构阶段被排除，不会以 `ColumnState.visible = false` 的形式保存。
- 同级 `order` 缺失时，使用当前声明顺序，并在归一化后输出连续、稳定的值。
- 是否有子列由合并后的子列数量决定。
- 子列缺省为空数组。
- 叶子列应拥有可恢复的 `width`、明确的 `widthManuallyChanged`，以及用于阻止重复二次分配的 `autoWidthLocked`。
- `fixed` 缺省表示使用当前列声明或默认不固定；`fixed: false` 表示显式取消固定。
- 旧数据缺少新增字段时，使用当前列声明、稳定默认值或运行时布局结果，不应导致列消失、排序异常或宽度状态失真。

当列禁止调整宽度时，列宽应以当前 `columns` 经运行时布局后的宽度为准，不使用旧持久化宽度继续影响当前列，同时清除“已被用户手动拖拽调整宽度”和“已锁定宽度”的状态。但归一化后的 `columnsState` 仍应保留该列当前可恢复的 `width`、`widthManuallyChanged: false` 和 `autoWidthLocked: false`。
...
`order` 的排序对象是当前列状态树中同一父节点下的完整同级序列，而不是只包含当前可见叶子列的序列。`visible: false` 的列不参与当前渲染和布局，但仍保留在可恢复的列状态中。

被 `colSpan` 覆盖的列没有独立操作入口，仅指内部 resize width 和 columns order 这类基于当前渲染命中的内置交互限制；外部业务仍可以基于稳定 key 给被覆盖列提供自己的操作入口。
...
- 初始化、当前容器宽度分配或旧状态缺少宽度时补齐的 `width` 是当前运行时布局快照，用于保证内部 committed state 和后续写回保持完整；它不代表用户手动调整，`widthManuallyChanged` 应为 `false`。
- `autoWidthLocked` 表达“用户提交的这批宽度已经消费过可用剩余空间”。用户拖拽和 placeholder auto fill 后应把它置为 `true`；初始化、当前容器宽度分配、旧状态缺少宽度时的 runtime width hydration 不应把它置为 `true`；当列 `resizeDisabled` 时应清除为 `false`。

第三类宽度补齐称为 runtime width hydration。它的边界是：

- 它可以更新内部 committed `columnsState`，因为补齐后的宽度需要参与后续写回，并在下次初始化中恢复。
...
- 如果当前列可以 resize，则可以恢复旧 `width`，即使旧宽度小于当前 `resizeMinWidth`。
- `widthManuallyChanged` 只在列可 resize 且旧状态明确为手动拖拽时保留。

只要列状态管线已启用，无论 `resizableColumns` 是否开启，所有可渲染叶子列都应拥有可恢复的 `width` 状态。旧状态只包含部分列宽，或之前未开放列宽交互时，也必须在 ready 或下一次写回前补齐完整宽度快照。

当当前可渲染叶子列只有一列时，运行时宽度分配应把容器剩余宽度直接分配给这唯一的叶子列，使表格宽度填满当前容器。这个规则属于运行时布局和 runtime width hydration 的宽度快照补齐，不代表用户手动调整宽度，也不应把 `widthManuallyChanged` 或 `autoWidthLocked` 置为 `true`。

拖拽 resize 和 placeholder auto fill 写回完整 `ColumnState` 时，应写入 `width`、`widthManuallyChanged` 和 `autoWidthLocked`，不会写入 `resizeMinWidth`。`onColumnsStateChange` 的第一个参数应是完整、归一化后的 `columnsState`；`info.patches` 只表达本次实际变化的用户字段，因此仍只包含 `width` 和 `widthManuallyChanged`。
...
- 开关变化后，下一次仍被允许的内置 UI 交互或 ref API 调用触发 `onColumnsStateChange` 时，应输出完整、可恢复的归一化快照，包括当前关闭开关对应的已有字段。
...
再例如 `visibleColumns` 关闭时，组件不展示内置显隐入口，业务调用 `setColumnVisible(key, false)` 也应返回 `false` 且不提交 patch；但已有 `visible: false` 仍然会参与恢复、完整快照和后续其它特性写回，渲染层按该状态过滤当前可见列。
...
列状态管线分为候选外观状态生成、最终快照生成、内部状态维护、运行时发布和外部写回几个阶段。

内部状态更新只维护表格自己的 committed state 或 preview draft。初始化、当前 `columns` 变化、`columnsStateKey` 变化、preview rebase、runtime width hydration 都属于这一类。它们可以让内部状态变得完整、稳定、可恢复，但默认不触发 `onColumnsStateChange`，因为它们不是一次业务提交的列配置变更。

UI 开关变化本身也是内部交互能力变化，不是数据提交；它不应触发 `onColumnsStateChange`，也不应清洗 committed state 或 draft。

最终快照生成是列状态管线的固定收口：候选外观状态必须先经过排序、宽度分配和 runtime width hydration，得到完整 `columnsState`、排序后的完整列状态、实际渲染树、flatten columns 和宽度结果。初始化、用户提交和 preview save 可以在得到最终快照后同时写入内部状态、发布运行时结果，并按语义触发 ready 或 change。

如果当前 columns、容器宽度、preview 状态等外部环境变化导致已有 committed state 或 draft 需要 rebase/hydration，管线可以更新内部状态，但必须在更新后结束本轮。不能在同一轮继续发布基于旧状态的运行时渲染结果；下一轮会从更新后的 active state 再生成最终快照。

运行时渲染结果只能由当前 active state 生成。宽度分配、ready 输出和 runtime width hydration 不能消费旧 committed state 生成出来的 `innerColumnsState`，也不能把实际渲染树当成新的 committed state 来源。

外部写回只发生在列配置被主动提交时，包括内置 UI 触发的 resize、placeholder auto fill、排序、显隐、固定，业务调用已启用特性对应 ref API 触发的命令式修改，以及有效 preview 会话中的 `saveColumnsStatePreview()`。触发写回时，`onColumnsStateChange(columnsState, info)` 的 `columnsState` 是完整、归一化、可恢复的快照，表达保存后的最终状态；`info.patches` 只表达本次提交中由用户主动操作产生的最终有效字段变化。对于 preview save，`info.patches` 来自预览期间累计并压缩后的用户 patch，而不是最终 draft 与 committed state 的完整结构差异。
...
内部 resize 和列排序会把 `colSpan` 覆盖到的叶子列展开成一组 key 后处理；展开范围限制在当前列状态树结构中同一父节点下的叶子列，跨父节点的列合并不作为列配置交互的承诺处理范围。

`colSpan` 覆盖关系在内部 resize width 和 columns order 中形成操作块。覆盖块的入口列负责承载这些内置交互，被覆盖列可以不单独渲染，但它仍然是当前列状态树中的真实叶子列。外部业务可以基于稳定 key 为被覆盖列提供单独操作入口，因此外部 ref API 不应把“被 `colSpan` 覆盖”直接等同于“永远只能跟随入口列操作”。

外部 ref API `setColumnVisible` 和 `setColumnFixed` 需要通过 options 提供 `coveredColSpan?: boolean`，用于控制本次操作是否覆盖被 `colSpan` 合并到入口列下的同父级叶子列。该字段默认 `true`；当目标 key 对应的当前列本身是叶子列、声明了有效的 `colSpan > 1`，并且 `coveredColSpan` 没有显式传入 `false` 时，会把同父级中被该 `colSpan` 覆盖到的叶子列纳入本次操作。业务如果希望只操作 `colSpan` 入口列自身，可以传入 `coveredColSpan: false`；如果希望单独操作被 `colSpan` 覆盖的列，可以直接传入该列自己的 key。

覆盖范围建议遵循以下基础规则：

- 普通叶子列只处理自身。
- `colSpan > 1` 的叶子列默认会处理它在当前列状态树结构中同一父节点下覆盖到的叶子列；传入 `coveredColSpan: false` 时只处理入口列自身。跨父节点的列合并不作为外部 ref API 的承诺处理范围。
- 如果目标 key 是被其它列 `colSpan` 覆盖的叶子列，调用仍只处理该 key 自身，不会反向扩展到 `colSpan` 入口列或其它被覆盖列。
- 传入分组列或树节点 key 时，属于该节点范围内的批量状态操作；具体覆盖范围按树结构和当前 API 的覆盖规则确定，渲染布局事实仍以当前可见叶子列为准。
- `visible: false` 的列不会出现在当前渲染结果中；如果业务需要操作它，应通过外部状态面板、完整 `columnsState` 或显式 key 调用 ref API。

`setColumnVisible` 受 `visibleColumns` 管控，`setColumnFixed` 受 `fixableColumns` 管控。`setColumnVisible(key, visible, options?)` 的 options 支持 `coveredColSpan?: boolean`。`setColumnFixed` 的 `insertPosition` 是可选的：不传 `insertPosition` 时，它只修改目标列固定状态，不修改 `order`；传入 `{ insertPosition: 'first' | 'last' }` 时，它会在修改固定状态的同时，按目标固定类型在当前列状态树结构中的同级列集合首尾位置重排 `order`。`setColumnFixed(key, fixed, options?)` 的 options 同时支持 `insertPosition?: 'first' | 'last'` 和 `coveredColSpan?: boolean`。这里的同级指拥有同一父节点的兄弟列，不是 flatten 后的全表叶子列；如果目标集合为空，则在该树结构同级范围内按起始固定列、未固定列、结束固定列的分组位置落点。对应特性开关关闭时，业务显式调用 ref API 也不应提交 patch，并返回 `false`；对应特性开启时，如果处于 preview 中，则写入 preview draft 并返回 `true`；如果不在 preview 中，则写入 committed state、触发 `onColumnsStateChange` 并返回 `true`。目标 key 不存在、当前列结构不允许修改、preview 模式过滤掉该 patch 或本次调用没有实际变化时，也应返回 `false`。
...
- 如果字段表达可恢复外观状态，应允许从 `ColumnState` 恢复，并在缺失时提供明确默认值或运行时补齐逻辑。
...
业务希望当前页面不展示显隐入口，但仍尊重用户之前隐藏过的列。关闭 `visibleColumns` 后，`visible: false` 仍参与恢复、完整快照和后续写回；用户只是不能通过内置入口继续修改显隐，当前渲染布局仍会过滤隐藏列。

### 关闭显隐特性后 ref API 不生效

业务关闭 `visibleColumns` 后，内置显隐入口和 `setColumnVisible` 都不应继续修改显隐状态。已有 `visible` 状态仍会恢复并影响渲染结果；如果业务希望外部面板继续修改显隐，需要保持 `visibleColumns` 开启，并在业务 UI 层隐藏或禁用表格内置入口。
...
旧 storage 中可能保存了用户调整过的宽度。当前列声明 `resizeDisabled` 后，合并结果应使用当前 `columns` 经运行时布局后的宽度，并清除用户手动拖拽宽度影响，但仍输出该列当前可恢复的宽度状态。

### 当前最小宽度变大

旧 storage 中保存的 `width` 可能小于当前列声明的 `resizeMinWidth`。恢复时仍应尊重旧 `width`，不把它抬到当前 `resizeMinWidth`，也不把 `resizeMinWidth` 写入 `ColumnState`。后续拖拽时，最小宽度约束会被当前宽度封顶；当用户把列宽放大到超过 `resizeMinWidth` 后，再缩小时才会重新受到该下限约束。
...
- `resizeDisabled` 是否阻断旧宽度继续影响当前列，并保留当前可恢复宽度状态。
- `autoWidthLocked` 是否在 resize 和 placeholder auto fill 后进入完整 `columnsState`，是否不会由 runtime width hydration 误置为 `true`，并在 `resizeDisabled` 时清除。
- `resizeMinWidth` 是否只来自当前 columns，且没有进入 `ColumnState`，也不会在恢复旧 `width` 时抬高列宽。
...
- runtime width hydration 导致内部 state 变化时，是否会在本轮状态更新后结束，而不是继续发布基于旧状态的渲染结果。
- 当前可渲染叶子列只有一列时，运行时宽度分配是否会把容器剩余宽度分配给该列，并且不会把该补齐视为用户手动宽度修改。
...
- preview save 是否按最终有效字段变化压缩用户 patch，例如 `setColumnVisible` 只输出 visible、`setColumnFixed` 不传 `insertPosition` 时只输出 fixed，传入 `insertPosition` 时可能输出 fixed 和/或 order，宽度只在用户拖拽或 auto fill 时输出。
