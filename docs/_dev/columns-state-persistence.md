...
- 普通叶子列只处理自身。
- `colSpan > 1` 的叶子列会处理它在当前列状态树结构中同一父节点下覆盖到的叶子列；跨父节点的列合并不作为外部 ref API 的承诺处理范围。
- 分组列仍按分组节点递归处理子列。

`setColumnVisible` 受 `visibleColumns` 管控，`setColumnFixed` 受 `fixableColumns` 管控。`setColumnFixed` 必须传入 `{ insertPosition: 'first' | 'last' }`，它会修改目标列固定状态，并按目标固定类型在当前列状态树结构中的同级列集合首尾位置重排 `order`。这里的同级指拥有同一父节点的兄弟列，不是 flatten 后的全表叶子列；如果目标集合为空，则在该树结构同级范围内按左固定列、不固定列、右固定列的分组位置落点。对应特性开关关闭时，业务显式调用 ref API 也不应提交 patch，并返回 `false`；对应特性开启时，如果处于 preview 中，则写入 preview draft 并返回 `true`；如果不在 preview 中，则写入 committed state、触发 `onColumnsStateChange` 并返回 `true`。目标 key 不存在、缺少 `insertPosition`、当前列结构不允许修改、preview 模式过滤掉该 patch 或本次调用没有实际变化时，也应返回 `false`。
...
- preview save 是否按最终有效字段变化压缩用户 patch，例如 `setColumnVisible` 只输出 visible、`setColumnFixed` 输出 fixed 和/或 order，宽度只在用户拖拽或 auto fill 时输出。
