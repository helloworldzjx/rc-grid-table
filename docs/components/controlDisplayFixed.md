...
`setColumnFixed` 通常只会修改传入 `key` 对应列的 `fixed`，并在传入 `insertPosition` 时修改该列的 `order`。本示例固定列操作始终以当前右键列的 `key` 作为目标 key；如果目标列是分组列，则该分组作为批量入口处理其子列集合。如果业务希望点击子列时改为操作顶层分组，可以自行根据 `ColumnInfo.ancestorKeys` 映射目标 key。

右键菜单会区分顶层列和非顶层列。只有顶层列可以通过固定操作子菜单选择插入位置：固定到左侧、固定到右侧和取消固定都可以选择目标固定类型列集合中的第一个或最后一个。非顶层列只提供固定到左侧、固定到右侧和取消固定，不传 `insertPosition`，因此只修改固定状态，不重排同级 `order`。已经处于左固定或右固定状态的顶层列仍可再次选择同一区域的第一个或最后一个，用于只调整顺序。
...
- `TableRef.setColumnVisible(key, visible, options?)`：设置指定列是否可见，返回是否写入成功。需要开启 `visibleColumns`；正常态立即提交，预览态写入草稿。`options.coveredColSpan` 默认为 `true`；当目标 key 对应的列本身声明了有效 `colSpan > 1`，且该字段没有显式传入 `false` 时，会同时处理被该 `colSpan` 覆盖到的同父级叶子列。
- `TableRef.setColumnFixed(key, fixed, options?)`：设置指定列固定状态，返回是否写入成功。需要开启 `fixableColumns`；`fixed` 可传 `'start'`、`'end'` 或 `false`，其中 `false` 表示取消固定；不传 `insertPosition` 时只修改 `fixed`，不重排 `order`；传入 `{ insertPosition: 'first' | 'last' }` 时，表示插入到目标固定类型在当前列状态树结构中的同级列集合第一个或最后一个，并可能同时重排 `order`。`options.coveredColSpan` 的语义与 `setColumnVisible` 相同。
...
`setColumnFixed` 传入树节点 key 时会以该树节点作为批量入口处理目标子列集合；目标集合由当前 API 覆盖规则决定。最终固定布局仍以当前可见叶子列的有效固定状态为准。本示例只有 `ancestorKeys.length === 0` 的顶层列会传入 `insertPosition`，非顶层列固定或取消固定时不会触发顺序重排。

如果叶子列声明了 `colSpan > 1`，本示例默认会把当前列状态树结构中同一父节点下被覆盖到的叶子列一起处理；传入 `coveredColSpan: false` 时才只处理入口列自身。被覆盖列仍是完整同级顺序中的节点，业务也可以基于稳定 key 为被覆盖列提供自己的外部操作入口；跨父节点的列合并不作为本示例的处理范围。
