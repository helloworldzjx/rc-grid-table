...
如果需要把固定列拖拽到非固定列区域排序，需要先取消该列固定状态，再在非固定列区域内拖拽排序。取消固定可以参考 [控制列显隐/固定](./control-display-fixed) 示例。
...
当拖拽中的列是固定列时，非固定列区域单元格会降低透明度，用来表示该区域不可达，也不会响应当前固定列拖拽的 over 或交换状态。
