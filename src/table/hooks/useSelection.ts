import { Key, MouseEvent, useCallback, useEffect, useMemo, useState } from 'react';
...
function useSelection<T = any>({
  ...
  const [innerSelectedRowKeys, setInnerSelectedRowKeys] = useState<Key[]>(() => {
    const initialKeys = rowSelection?.selectedRowKeys ?? rowSelection?.defaultSelectedRowKeys ?? [];
    return rowSelection?.type === 'radio' ? initialKeys.slice(0, 1) : initialKeys;
  });
  ...
  const selectedRowKeys = useMemo(() => {
    const keys = controlled ? rowSelection?.selectedRowKeys ?? [] : innerSelectedRowKeys;
  ...
  const recordEntities = useMemo(() => {
    ...
    const traverse = (records: T[], parentKey?: Key) => {
      records.forEach((record) => {
        ...
        keyDisabledMap.set(key, !!rowSelection?.getCheckboxProps?.(record)?.disabled);
        keyChildrenMap.set(key, children.map((child) => getRecordKey(child, rowKey)));
        if (children.length) {
  ...
  const getSelectedRows = useCallback((keys: Key[]) => {
  ...
  const updateSelectedKeys = useCallback((keys: Key[], infoType: 'single' | 'all' | 'none') => {
    const nextKeys = type === 'radio' ? keys.slice(0, 1) : uniqKeys(keys);
    if (!controlled) {
      setInnerSelectedRowKeys(nextKeys);
    }
    rowSelection?.onChange?.(nextKeys, getSelectedRows(nextKeys), { type: infoType });
    return nextKeys;
  }, [controlled, getSelectedRows, rowSelection, type]);

  const collectDescendantKeys = useCallback((key: Key): Key[] => {
  ...
  const selectedKeySet = useMemo(() => {
    ...
    selectedRowKeys.forEach((key) => {
      collectDescendantKeys(key).forEach((descendantKey) => nextKeySet.add(descendantKey));
    });
    return nextKeySet;
  }, [checkStrictly, collectDescendantKeys, selectedRowKeys, type]);

  const conductSelect = useCallback((record: T, selected: boolean) => {
    const key = getRecordKey(record, rowKey);
    const targetKeys = checkStrictly ? [key] : [key, ...collectDescendantKeys(key)];
    const nextKeySet = new Set(selectedRowKeys);
    ...
    if (!checkStrictly) {
      ...
      while (parentKey !== undefined) {
        ...
        if (descendantKeys.length && descendantKeys.every((descendantKey) => nextKeySet.has(descendantKey))) {
  ...
  }, [checkStrictly, collectDescendantKeys, recordEntities.keyParentMap, rowKey, selectedRowKeys]);
  ...
  const isSelected = useCallback((record: T) => {
    ...
    if (type === 'checkbox' && !checkStrictly) {
      ...
      return descendantKeys.length > 0 && descendantKeys.every((descendantKey) => selectedKeySet.has(descendantKey));
    }
    return false;
  }, [checkStrictly, collectDescendantKeys, rowKey, selectedKeySet, type]);

  const isHalfSelected = useCallback((record: T): boolean => {
    ...
    return descendantKeys.some((descendantKey) => selectedKeySet.has(descendantKey));
  }, [checkStrictly, collectDescendantKeys, isSelected, rowKey, selectedKeySet, type]);
  ...
  }, [recordEntities.allKeys, recordEntities.keyDisabledMap, selectAllMode, type]);

  const isAllSelected = useMemo(() => {
    return type === 'checkbox' && enabledKeys.length > 0 && enabledKeys.every((key) => selectedKeySet.has(key));
  }, [enabledKeys, selectedKeySet, type]);

  const isPartiallySelected = useMemo(() => {
    return type === 'checkbox' && !isAllSelected && enabledKeys.some((key) => selectedKeySet.has(key));
  }, [enabledKeys, isAllSelected, selectedKeySet, type]);

  const onSelectRecord = useCallback((record: T, rowIndex: number, nativeEvent: MouseEvent<HTMLElement>) => {
    const key = getRecordKey(record, rowKey);
    const disabled = type === 'radio'
    ...
    const nextKeys = type === 'radio'
      ? selected ? [key] : selectedRowKeys
      : conductSelect(record, selected);
    const mergedKeys = updateSelectedKeys(nextKeys, 'single');
    rowSelection?.onSelect?.(record, selected, getSelectedRows(mergedKeys), nativeEvent);
  }, [
    conductSelect,
    getSelectedRows,
    rowKey,
  ...
  const onSelectAll = useCallback((nativeEvent: MouseEvent<HTMLElement>) => {
    ...
    if (isAllSelected) {
      ...
      const nextKeys = selectAllMode === 'enabled'
  ...
  }, [enabledKeys, isAllSelected, selectedRowKeys, selectAllMode, type, updateSelectedKeys]);
